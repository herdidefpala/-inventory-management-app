...
Seret seluruh folder `stock-opname-app` ke halaman tersebut
Cara via Git (direkomendasikan untuk update berkelanjutan):
Push folder ini ke repo GitHub/GitLab
Di Netlify: Add new site → Import an existing project
...
Tulis data — ganti setiap `saveState()` + mutasi array dengan
`supabase.from(...).insert/update/delete`, aktifkan Row Level Security
sesuai matriks di halaman Pengaturan
Audit log — buat sebagai tabel append-only (policy tanpa `DELETE`)
agar aturan "tidak bisa dihapus dari UI" berlaku juga di level database
Import wizard — sambungkan ke parser XLSX sungguhan (mis. SheetJS)
